    ...
    <scroll-view
      ...
      <view
        ...
        <view class="buyer-info">
          ...
          <text class="buyer-name">{{ order.buyer?.nickname || '未知用户' }}</text>
        </view>
        ...
        <view class="order-footer">
          <text class="order-amount">
            合计：<text class="amount-value">¥{{ order.actualAmount?.toFixed(2) }}</text>
          </text>
...
import { getSupplierOrders, acceptOrder, rejectOrder, shipOrder } from '@/api/order';
...
function goDetail(id: number) {
  uni.navigateTo({ url: `/pages/supplier/order-detail?id=${id}` });
}
...
async function loadOrders() {
  ...
  try {
    ...
    const data = await getSupplierOrders(params);
    if (data?.list) {
      if (page.value === 1) {
...
function handleReject(order: any) {
  uni.showModal({
    title: '拒单原因',
    editable: true,
    ...
    success: async (res) => {
      if (res.confirm && res.content) {
        try {
          await rejectOrder(order.id, res.content);
